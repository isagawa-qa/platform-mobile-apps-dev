...
At the start of each Claude Code session (the first task you receive), or when the user says "continue" after a restart:
...
**First action of a session = /kernel/session-start.** Later tasks in the same session do NOT re-run session-start or force a re-anchor; the hook re-anchors every `actions_limit` (30) actions.
...
WORK:
  ...
  3. Every 30 actions (actions_limit) → hook blocks → /kernel/anchor
  4. Run tests
...
**Auto Counter:** Hook automatically tracks Write, Edit, AND Bash. Blocks after 30 actions (configurable via `actions_limit` in `[domain]_workflow.json`). You do NOT need to increment manually.
