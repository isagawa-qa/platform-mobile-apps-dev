...
- Verify a device is reachable for the chosen platform and device_location:
...
- Verify the host OS can run the platform:
    ...
    named. Route it to a device_location that reaches a machine which HAS the tooling -
...
- Device: whatever the resolved device_location provides; never headless — a simulator
...
- If validation FAIL: RE-ASK credential question, or report the device
  problem and WAIT — do not silently switch device_location
```
...
| Device Location supports discovery | Must be an interactive device location, never `ci` | REPORT; offer `remote` or `cloud` (§ I) |
...
✓ Step 2: Pre-flight
  ...
  • Device: iPhone 16 Pro / iOS 18.6 (device_location: ci)
  • Appium: http://127.0.0.1:4723 responding
```

---

## I. Which Device Location Can Do What

**Discovery and execution are different jobs and not every device location serves both.**
...
| Device Location | Device | Discovery (Step 4) | Execution (Step 5) |
|-------|--------|--------------------|--------------------|
...
| Your host | Target | Discovery device location |
|-----------|--------|-----------------|
| macOS | iOS | `local` |
...
**Artifact kinds are not interchangeable**, and choosing the device location chooses the
artifact:
...
```
  Verify device reachable for platform + device_location
      │
  ┌───┴───┐
  ▼       ▼
 OK    NOT OK → REPORT + WAIT (never switch device_location silently)
  │
  ▼
